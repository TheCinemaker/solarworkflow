...
export async function exportProjectPDF(projectId, onProgress) {
  ...
  doc.text('VoltDesk', margin, 11);

  // Generated by VoltDesk felirat
  doc.setFontSize(7);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(180, 200, 255);
  doc.text('Generated by VoltDesk', margin, 17);

  // Generálás dátuma jobb oldalon
  doc.setFontSize(8);
  doc.setTextColor(140, 150, 180);
  doc.text(normalizeText(`Készítve: ${formatDate(new Date().toISOString())}`), pageW - margin, 13, { align: 'right' });
