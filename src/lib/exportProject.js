...
export async function exportProjectPDF(projectId, onProgress) {
  ...
  const addFooter = () => {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        normalizeText(`Generálta: VoltDesk · ${formatDate(new Date().toISOString())} · Oldal ${i}/${pageCount}`),
  ...
  doc.text('VoltDesk', margin, 18);

  // Projekt szám
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(180, 200, 255);
  doc.text(normalizeText(`Projekt Munkalap · ${project.serial_number || 'N/A'}`), margin, 26);

  // Generálás dátuma jobb oldalon
  doc.setFontSize(8);
  doc.setTextColor(140, 150, 180);
  doc.text(normalizeText(`Készítve: ${formatDate(new Date().toISOString())}`), pageW - margin, 18, { align: 'right' });
  
  // Státusz
  const statusText = project.archived ? 'LEZART' : 'AKTIV';
  const statusColor = project.archived ? [46, 209, 88] : [79, 142, 247];
  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...statusColor);
  doc.text(normalizeText(statusText), pageW - margin, 26, { align: 'right' });
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(60, 60, 60);
  doc.text(normalizeText(`Készültség: ${progressPercent}%`), margin, y);
  ...
  if (progressPercent > 0) {
    const fillColor = progressPercent === 100 ? [46, 209, 88] : [79, 142, 247];
    doc.setFillColor(...fillColor);
    doc.roundedRect(margin, y, barWidth * (progressPercent / 100), barHeight, 2, 2, 'F');
  }
  ...
  if (tasks.length > 0) {
    ...
    tasks.forEach((task) => {
      ...
      // Checkbox
      if (isDone) {
        doc.setFillColor(46, 209, 88);
        doc.roundedRect(margin, y - 3, 4, 4, 0.5, 0.5, 'F');
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(6);
        doc.setTextColor(255, 255, 255);
        ...
        doc.roundedRect(margin, y - 3, 4, 4, 0.5, 0.5, 'S');
      }
      ...
      const taskLines = doc.splitTextToSize(normalizeText(task), contentW - 10);
